...
class CaptchaHelper
{
    ...
    // 验证码字体颜色
    private int|null $color = null;
    // 验证码字符集合
    private string $codeSet = '2345678abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXY';
    // 启用数字验证码字符集合
    protected bool $numberCode = false;
    // 数字验证码字符集合
    private string $numberCodeSet = '0123456789';
    // 验证码过期时间（s）
    ...
    // 执行垃圾收集的频率
    protected int $gcFreq = 10;
    // 图像上的噪波点数
    protected int $dotNoiseLevel = 100;
    // 启用噪波点数
    protected bool $useNoiseLevel = true;
    // 图像上的噪波线数
    protected int $lineNoiseLevel = 10;
    // 启用噪波线数
    protected bool $useLineNoiseLevel = true;
    // 图像后缀（包括点）
     ...
     */
    public static function instance(): CaptchaHelper
    {
        return new self();
    }
     ...
     * @throws Exception
     */
    public static function image(array $config = []): array
    {
        ...
        if (!function_exists("imagettftext")) {
            throw new Exception("Image CAPTCHA requires TTF fonts support");
        }

        return (new self())->create($config);
    }
     ...
     */
    protected function configure(array $config = null): void
    {
    ...
    protected function generate(): array
    {
        ...
        if ($this->math) {
            ...
            if ($this->useZh) {
                ...
                if ($this->numberCode) {
                    $characters = str_split($this->numberCodeSet);
                } else {
        ...
        return [
            'value' => $bag,
            'key' => $hash,
            'name' => Helper::uuid()::uuid4()->toString() . $this->suffix,
        ];
    }
     ...
     * @throws Exception
     */
    protected function create(array $config = null): array
    {
        ...
        // 验证码使用随机字体
        $ttfPath = dirname(__DIR__, 1) . '/data/captcha/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        // 保存验证码
        if (!empty($this->imgDir) && file_exists($this->imgDir)) {
            file_put_contents("{$this->imgDir}" . DIRECTORY_SEPARATOR . $generator['name'], $content);
        } else {
            $generator['size'] = strlen($content);
    ...
    protected function writeNoise(): void
    {
        ...
        if ($this->useNoiseLevel) {
            for ($i = 0; $i < $this->dotNoiseLevel; $i++) {
                imagefilledellipse($this->im, mt_rand(0, (int)$this->imageW), mt_rand(0, (int)$this->imageH), 2, 2, $noiseColor);
            }
        }
        if ($this->useLineNoiseLevel) {
            for ($i = 0; $i < $this->lineNoiseLevel; $i++) {
                imageline($this->im, mt_rand(0, (int)$this->imageW), mt_rand(0, (int)$this->imageH), mt_rand(0, (int)$this->imageW), mt_rand(0, (int)$this->imageH), $noiseColor);
            }
        }
    }
    ...
    {
        $path = dirname(__DIR__, 1) . '/data/captcha/bgs/';
        $dir = dir($path);
    ...
    protected function gc(): void
    {
        ...
        if (!$this->imgDir || strlen($this->imgDir) < 2) {
            return;
        }
        ...
        foreach (new DirectoryIterator($this->imgDir) as $file) {
